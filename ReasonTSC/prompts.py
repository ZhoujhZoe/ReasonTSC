...
Your second task is to analyze the time series data and refine your understanding based on the 
classification results and logits (model probabilities for each category) provided by a domain-specific model.
''',
    
    "third_round_description": '''
        ### Task Description:
        Based on your refined understanding, your third task is to perform the time series classification task 
        on the new data sample. You will use your updated analysis of time series patterns along with the 
        result and category logits (model probabilities for each category) from the domain-specific model 
        to make a final classification decision.
    ''',
    
    "first_round_task": '''
        ### Analysis Task
        Compare and summarize the significant differences in the time series patterns across categories 
        based on the following characteristics. Explicitly state if no differences are observed. 
    ...
    "third_round_task": '''
        ...
        1. **Analyze the Time Series Pattern**: [Examine the time series data for trends, cyclic behavior, 
        stationarity, amplitude, rate of change, and outliers. Compare these characteristics across 
        the categories to identify any significant patterns or differences.]
        2. **Interpret the Model's Results**: [Evaluate the model's classification result and logits. 
        Assess the confidence level of the model's prediction and how well it aligns with the observed 
        time series patterns.]
        3. **Make a Preliminary Prediction**: [Based on your analysis of the time series pattern and the 
        model's results, make an initial classification decision. Provide a brief explanation for this decision.]
        4. **Review Alternative Classifications**: [Consider if there are any other plausible categories 
